...
struct MemoryEditor
{
    ...
    int             Cols;                                       // = 8     // number of columns to display.
    ...
    // Function handlers
    ImU16            (*ReadFn)(const ImU16* mem, size_t off, void* user_data);      // = 0      // optional handler to read bytes.
    void            (*WriteFn)(ImU16* mem, size_t off, ImU16 d, void* user_data);   // = 0      // optional handler to write bytes.
    bool            (*HighlightFn)(const ImU16* mem, size_t off, void* user_data); // = 0      // optional handler to return Highlight property (to support non-contiguous highlighting).
    ImU32           (*BgColorFn)(const ImU16* mem, size_t off, void* user_data);   // = 0      // optional handler to return custom background color of individual bytes.
    ...
    void CalcSizes(Sizes& s, size_t mem_size, size_t base_display_addr)
    {
        ...
        s.HexCellWidth = (float)(int)(s.GlyphWidth * 4.5f);             // "FFFF " we include trailing space in the width to easily catch clicks everywhere
        ...
        while (clipper.Step())
            for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
            {
                ...
                for (int n = 0; n < Cols && addr < mem_size; n++, addr++)
                {
                    ...
                    {
                        is_next_byte_highlighted = (addr + 1 < mem_size) && ((HighlightMax != (size_t)-1 && addr + 1 < HighlightMax) || (HighlightFn && HighlightFn(mem_data, addr + 1, UserData)));
                    ...
                    if (DataEditingAddr == addr)
                    {
                        ...
                        ImSnprintf(input_text_user_data.CurrentBufOverwrite, 5, format_byte, ReadFn ? ReadFn(mem_data, addr, UserData) : mem_data[addr]);
                        ...
                        ImGui::SetNextItemWidth(s.GlyphWidth * 4);
                        ...
                        DataEditingTakeFocus = false;
                        if (input_text_user_data.CursorPos >= 4)
                            data_write = data_next = true;
                        ...
                        {
                            if (WriteFn)
                                WriteFn(mem_data, addr, (ImU16)data_input_value, UserData);
                            else
                    ...
                    else
                    {
                        ...
                        ImU16 b = ReadFn ? ReadFn(mem_data, addr, UserData) : mem_data[addr];
